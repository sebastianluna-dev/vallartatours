...
- El charter no tiene horario fijo: sus paradas son minutos desde la salida (`itineraryOffsets` en `constants/services.const.ts`), así que el itinerario pone la hora real de la que elige la tarjeta de reserva; las dos la comparten con `departure.context.tsx`.
...
- **`components/site/sections/`**: `shell/` (cabecera con navegación activa y selector de idioma, menú del teléfono —donde vive el botón de reservar—; pie) y una carpeta por página: `home/` (hero con buscador, carrusel, tres pasos, cifras y reseñas), `services/` (hero y paneles), `service/` (hero con la tarjeta de reserva, itinerario, incluido, reseñas, preguntas) `contact/` (hero, formulario y canales) y `legal/` (el documento con su índice pegajoso). Cada `.section.tsx` o `.comp.tsx` importa su propio `.css`.
- **`components/site/shared/`**: iconos, logo, hero de página, carrusel de servicios (escritorio y teléfono con un solo DOM; en escritorio tres copias de cada servicio para que el mazo sólo avance hacia la izquierda), selector de idioma, redes, el ornamento de olas y `Reveal`, que aparece un elemento la primera vez que llega a la pantalla renderizando su misma etiqueta.
- **`hooks/`**: `use-carousel.hook.ts` (índice circular, flechas, swipe y qué fotos del escenario vale la pena cargar), `use-reveal.hook.ts` (un `IntersectionObserver` que dispara una sola vez) y `use-in-view.hook.ts` (el mismo, pero respondiendo mientras se hace scroll: con medio carrusel en pantalla las flechas del teclado lo mueven).
